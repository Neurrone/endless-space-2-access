...
Each bearing announces the share: of every tile the probe would light up — its flight out to its reach, widened by its vision to either side — how much is already explored (`Southeast: 35 percent explored`), so one number is enough to skip a bearing or keep listening. The detail is in the bearing's [review buffer](buffers.md), one line per clause: the share, the unexplored stretches along the flight line in map units out to the map edge, and any fog lying just beside the line (`Unexplored alongside to the southwest: 1-4`). The group announces the probe's reach once (`Launch probe, reach 30`), so a stretch beyond that number is one this probe will not get to — the stretches describe what is out there; the share describes what this launch would buy. A bearing with nothing left to find holds only `Fully explored to the map edge` behind its share.
