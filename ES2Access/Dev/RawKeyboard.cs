...
namespace ES2Access.Dev
{
    ...
    /// being down is therefore invisible to it: the consumed-key latch the game's scans ask about,
    /// <c>UnityEngine.Input.anyKeyDown</c>, the engine's own KeyDown delivery to the focused control,
    /// and the "was this Return still down when the focus left" question a commit is decided by
    /// (<see cref="Screens.TextFieldEditor.FocusLeaving"/>). Those are exactly the seams a text box
    /// lives at, and until this route they could only be tested by hand - which is how a rename box
    /// that committed on the FIRST Enter shipped twice.
    ///
