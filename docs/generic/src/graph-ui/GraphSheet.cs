...
namespace ES2Access.Core.UI
{
    ...
    public sealed class GraphSheet
    {
        ...
        /// <summary>
        /// Everything one sheet declared for a set of rows: the node declarations in the order they
        /// were emitted, and the edges that wire them. Kept by a caller whose rows the game has not
        /// rebound, and handed back to <see cref="Replay"/> on the frames in between.
        ///
        ...
        /// drawing it. What a block cannot notice is a row whose SHAPE changed under a binding the
        /// game kept - which is the caller's business to key on.
        /// </summary>
        ...
        /// <summary>Keep everything the rows emitted from here on, so that a caller whose rows have not
        /// been rebound can hand it back to <see cref="Replay"/> instead of reading the table again.
        /// Set it before the first row; the block's <see cref="Block.FirstRow"/> is filled by
        ...
        /// <summary>Re-declare a kept <see cref="Block"/> into this sheet's builder, inside whichever
        /// region is open. The sheet's own row state stays empty - a replayed sheet emits no further
        /// rows - and <see cref="FirstRow"/> answers what the recorded build answered.</summary>
        public GraphSheet Replay(Block block)
        {
            if (block == null) return this;
            _b.Replay(block.Nodes, block.Edges);
