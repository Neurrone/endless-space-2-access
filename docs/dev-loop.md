...
| `DevProbe.TooltipParity()` | The promised/misaimed/unraised/uncovered/misclassed tooltip self-check on the FOCUSED screen, ten buckets (six findings, four context), aim read off `NodeVtable.PointsAt` rather than re-derived; `unraised` is the one that asks whether the pointer is ever MOVED there, `misclassed` the one that asks whether the tooltip's KIND decided how it reached the player | `ES2Access/Dev/TooltipAudit.cs` |
