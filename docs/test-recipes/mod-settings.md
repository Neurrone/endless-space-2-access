...
first row's name cell — "Controls, table, Confirm, Enter, ⟨description⟩, 1 of 57" — and then
...
in ("Cancel, empty, button, 2 of 57"). Cell ids are `options:⟨panel⟩/keys/row⟨hash⟩c⟨0|1|2⟩`.
**The row count is 57** — one per `action.*.title` key in the locale, and the same figure the
physical-key measurements below read back ("46 of 57").
