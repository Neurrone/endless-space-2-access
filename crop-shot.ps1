param(
    # x,y,w,h of the region to keep, in screen pixels (the rects /gui/age and DevProbe report).
    [Parameter(Mandatory = $true)][int[]]$Rect,
    [string]$Out,
    [int]$Margin = 12,
    [string]$DevUrl = 'http://127.0.0.1:8771'
)
...
$tmp = Join-Path $env:TEMP "es2access-shot.png"
